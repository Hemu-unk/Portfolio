...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const getWebGLContext = (cnv) => {
      ...
      let gl = cnv.getContext("webgl2", params);
      if (!gl) {
        gl = cnv.getContext("webgl", params) || cnv.getContext("experimental-webgl", params);
      }
    ...
    if (!ext.supportLinearFiltering) {
      localConfig.DYE_RESOLUTION = 256;
      localConfig.SHADING = false;
    }
    ...
    const displayShaderSource = `
      ...
      void main () {
          vec3 c = texture2D(uTexture, vUv).rgb;
              ...
              // Compress high values so intense splats don't wash out to bright white.
              c = c / (1.0 + c);
              c = pow(c, vec3(1.15));
    ...
    const initFramebuffers = () => {
      const simRes = getResolution(localConfig.SIM_RESOLUTION);
      const dyeRes = getResolution(localConfig.DYE_RESOLUTION);
    ...
    const updateKeywords = () => {
      ...
      if (localConfig.SHADING) displayKeywords.push("SHADING");
    ...
    const generateColor = () => {
      const palette = Array.isArray(COLOR_PALETTE) && COLOR_PALETTE.length > 0 ? COLOR_PALETTE : DEFAULT_COLOR_PALETTE;
      const swatch = palette[Math.floor(Math.random() * palette.length)];
      const jitter = (value) => Math.min(1, Math.max(0, value + (Math.random() - 0.5) * 0.04));
      return {
        r: jitter(swatch.r),
        g: jitter(swatch.g),
        b: jitter(swatch.b),
    ...
    const splat = (x, y, dx, dy, color) => {
      ...
      if (splatProgram.uniforms.radius) gl.uniform1f(splatProgram.uniforms.radius, correctRadius(localConfig.SPLAT_RADIUS / 100));
      blit(velocity.write);
      velocity.swap();
    ...
    const splatPointer = (pointer) => {
      const dx = pointer.deltaX * localConfig.SPLAT_FORCE;
      const dy = pointer.deltaY * localConfig.SPLAT_FORCE;
    ...
    const drawDisplay = (target) => {
      ...
      if (localConfig.SHADING && displayMaterial.uniforms.texelSize) {
    ...
    const step = (dt) => {
      ...
      if (vorticityProgram.uniforms.curl) gl.uniform1f(vorticityProgram.uniforms.curl, localConfig.CURL);
      ...
      if (clearProgram.uniforms.value) gl.uniform1f(clearProgram.uniforms.value, localConfig.PRESSURE);
      blit(pressure.write);
      pressure.swap();
      ...
      for (let i = 0; i < localConfig.PRESSURE_ITERATIONS; i++) {
      ...
      if (advectionProgram.uniforms.dissipation) gl.uniform1f(advectionProgram.uniforms.dissipation, localConfig.VELOCITY_DISSIPATION);
      blit(velocity.write);
      velocity.swap();
      ...
      if (advectionProgram.uniforms.dissipation) gl.uniform1f(advectionProgram.uniforms.dissipation, localConfig.DENSITY_DISSIPATION);
      blit(dye.write);
      dye.swap();
    };
    ...
    const updateColors = (dt) => {
      colorUpdateTimer += dt * localConfig.COLOR_UPDATE_SPEED;
